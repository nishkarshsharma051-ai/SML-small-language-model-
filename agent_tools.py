...
def execute_python_code(code: str) -> str:
    """Executes Python code in a sandbox process with output capture, Matplotlib plot rendering, and timeout."""
    import sys
    import tempfile
    import uuid
    import time
    ...
    try:
        ...
        # Inject Matplotlib headless config and auto-save setup if matplotlib is used
        wrapped_code = (
            "import os\n"
            "try:\n"
            "    import matplotlib\n"
            ...
            "    plt = None\n\n"
            f"{code}\n\n"
            "if 'plt' in locals() and plt is not None and getattr(plt, 'get_fignums', lambda: [])():\n"
            f"    fig_path = os.path.join('{images_dir}', 'chart_{int(time.time())}_{uuid.uuid4().hex[:6]}.png')\n"
            "    plt.savefig(fig_path, bbox_inches='tight', dpi=150)\n"
            "    print(f'__INLINE_IMAGE__:' + os.path.basename(fig_path))\n"
        )
        ...
        try:
            ...
            if result.stdout:
                for line in result.stdout.splitlines():
                    if line.startswith("__INLINE_IMAGE__:"):
                        img_name = line.split(":", 1)[1].strip()
                        image_tags.append(f"\n\n![Generated Chart](/generated_images/{img_name})")
                    else:
            ...
            if stdout_lines:
                output += f"Output:\n" + "\n".join(stdout_lines) + "\n"
            if result.stderr:
                # Filter out matplotlib headless warnings
                clean_err = "\n".join([l for l in result.stderr.splitlines() if "UserWarning" not in l and "Font" not in l])
                if clean_err.strip():
                    output += f"Error output:\n{clean_err.strip()}\n"
                    
            if image_tags:
                output += "".join(image_tags)
                
            if not output.strip():
                output = "Code executed successfully with no text output."
            return output[:4000]
        except subprocess.TimeoutExpired:
            return "Execution Error: Python execution timed out (limit: 10 seconds)."
        finally:
...
def generate_markdown_report(title: str, content: str) -> str:
    """Exports a structured Markdown research report into the data/reports directory for download."""
    import time
    try:
        reports_dir = os.path.join(os.getcwd(), "data", "reports")
        os.makedirs(reports_dir, exist_ok=True)
        safe_title = "".join([c if c.isalnum() else "_" for c in title.lower()])[:40]
        filename = f"report_{safe_title}_{int(time.time())}.md"
        filepath = os.path.join(reports_dir, filename)
        
        full_report = f"# {title}\n\n*Generated by Ting Ling Ling AI Assistant on {time.strftime('%Y-%m-%d %H:%M:%S')}*\n\n---\n\n{content}"
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Exports a structured Markdown research paper or essay file into data/reports for download.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
                    ...
                    "content": {
                        "type": "string",
                        "description": "Complete markdown text content of the report."
                    }
                },
